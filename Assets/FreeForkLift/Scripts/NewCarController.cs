...
public class NewCarController : MonoBehaviour {
  ...
  private void FixedUpdate() {
    if (Input.GetKey(KeyCode.LeftShift)) {
      if (1f - multiplier < 0.01f) {
        multiplier = 1f;
      } else {
        multiplier = Mathf.Lerp(multiplier, 1f, 0.05f);
      }
    } else {
      if (multiplier - 0.5f < 0.01f) {
        multiplier = 0.5f;
      } else {
        multiplier = Mathf.Lerp(multiplier, 0.5f, 0.002f / Mathf.Pow((multiplier - 0.5f), 0.5f));
      }
    }
  }
...
*/
  ...
  private void CapSpeed() {
    ...
    switch (m_SpeedType) {
      case SpeedType.MPH:
        speed *= 2.23693629f;
        if (speed > (m_Topspeed * multiplier))
          m_Rigidbody.velocity = ((m_Topspeed * multiplier) / 2.23693629f) * m_Rigidbody.velocity.normalized;
        break;

      case SpeedType.KPH:
        speed *= 3.6f;
        if (speed > (m_Topspeed * multiplier))
          m_Rigidbody.velocity = ((m_Topspeed * multiplier) / 3.6f) * m_Rigidbody.velocity.normalized;
        break;
    }
  }
